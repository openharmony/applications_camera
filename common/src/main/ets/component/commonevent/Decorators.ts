...
const TAG: string = 'Decorators';
/* instrument ignore file */
export function debounce(timeout: number) {
  return function inner(target: CommonEventManager, propKey: string, descriptor: PropertyDescriptor): void {
    let curFunc: number = 0;
    const original = descriptor.value;
    descriptor.value = function (...args: string[]): void {
      HiLog.i(TAG, `debounce invoke ${propKey} curFunc: ${curFunc}.`);
...
export function throttle(waitTime: number) {
  return function inner(target: CommonEventManager, propKey: string, descriptor: PropertyDescriptor): void {
    ...
    descriptor.value = function (...args: string[]): void {
      ...
      HiLog.i(TAG, `throttle invoke ${propKey} timeInterval: ${curTime - lastTime}.`);
