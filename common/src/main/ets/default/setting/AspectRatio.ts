...
export default class AspectRatio {
  ...
  public static getPhotoPreviewSize(platform: CameraPlatformCapability, cameraId: CameraId, aspectRatio: string) {
    ...
    CLog.info(`${this.TAG} getPhotoPreviewSize size = ${JSON.stringify(platform.mPhotoPreviewSizeMap[cameraId][index])}`)
    return platform.mPhotoPreviewSizeMap[cameraId][index]
  }

  public static getImageSize(platform: CameraPlatformCapability, cameraId: CameraId, aspectRatio: string) {
    let index = AspectRatio.getIndex(aspectRatio)
    CLog.info(`${this.TAG} getImageSize size = ${JSON.stringify(platform.mImageSizeMap[cameraId][index])}`)
    return platform.mImageSizeMap[cameraId][index]
  }
}