...
export default class Resolution {
  ...
  public static getVideoPreviewSize(platform: CameraPlatformCapability, cameraId: CameraId, resolution: string) {
    ...
    CLog.info(`${this.TAG} getVideoPreviewSize size = ${JSON.stringify(platform.mVideoPreviewSizeMap[cameraId][index])}`)
    return platform.mVideoPreviewSizeMap[cameraId][index]
  }

  public static getVideoFrameSize(platform: CameraPlatformCapability, cameraId: CameraId, resolution: string) {
    let index = Resolution.getIndex(resolution)
    CLog.info(`${this.TAG} getVideoFrameSize index = ${index}`)
    CLog.info(`${this.TAG} getVideoFrameSize size = ${JSON.stringify(platform.mVideoFrameSizeMap[cameraId][index])}`)
    return platform.mVideoFrameSizeMap[cameraId][index]
  }
}